...
public class NimbleService {
    @Value(value="${api.nimble.url}")
    private String rootUrl;
    @Value(value="${api.nimble.username}")
    private String username;
    @Value(value="${api.nimble.password}")
    ...
    public HttpResponse<String> updatePayment(String requestId, String referenceNo, double amount, String narration, String serviceCode) throws Exception {
        HashMap<String, Object> coreRequest = new HashMap<String, Object>();
    ...
    public boolean validateReference(String requestId, String referenceNo) throws Exception {
        HashMap<String, String> coreRequest = new HashMap<String, String>();
        coreRequest.put("requestId", requestId);
        coreRequest.put("referenceNo", referenceNo);
        String payload = this.mapper.writeValueAsString(coreRequest);
    ...
    private HttpResponse<String> postRequest(String url, String payload) throws Exception {
        HttpRequest request = HttpRequest.newBuilder().uri(new URI(this.rootUrl + url)).header("Content-Type", "application/json").header("Authorization", "Bearer " + this.jwtToken).timeout(Duration.ofSeconds(60L)).POST(HttpRequest.BodyPublishers.ofString(payload, StandardCharsets.UTF_8)).build();
        this.logger.trace("PAYLOAD - {}", (Object)payload);
        System.out.println(payload);
        HttpResponse<String> response = this.client.send(request, HttpResponse.BodyHandlers.ofString());
        System.out.println(response);
        this.logger.trace("REQUEST - {}", response);
        this.logger.trace("RESPONSE - {}", (Object)response.body());
        return response;
    }

    private HttpResponse<String> authUser(String username, String password) throws Exception {
        HttpRequest request = HttpRequest.newBuilder().uri(new URI(this.rootUrl + "/authenticate")).header("Content-Type", "application/json").timeout(Duration.ofSeconds(60L)).POST(HttpRequest.BodyPublishers.ofString(this.mapper.writeValueAsString(Map.of("username", username, "password", password)), StandardCharsets.UTF_8)).build();
        HttpResponse<String> response = this.client.send(request, HttpResponse.BodyHandlers.ofString());
        this.logger.trace("AUTH REQUEST - {}", (Object)response.toString());
        return response;
    }

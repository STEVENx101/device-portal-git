    ...
    public Map<String, Object> fetchDpdBucketReport(Map<String, Object> filters) {
        ...
        String sql = String.format(
                """
                            SELECT
                                %s AS category_name,
                                COUNT(CASE WHEN COALESCE(p1.dpd, 0) = 0 AND COALESCE(p1.loan_status, '') <> 'N' THEN 1 END) AS dpd0_count,
                                SUM(CASE WHEN COALESCE(p1.dpd, 0) = 0 AND COALESCE(p1.loan_status, '') <> 'N' THEN COALESCE(p1.exposure, l.loan_amount, 0) ELSE 0 END) AS dpd0_val,
                                COUNT(CASE WHEN COALESCE(p1.dpd, 0) BETWEEN 1 AND 30 AND COALESCE(p1.loan_status, '') <> 'N' THEN 1 END) AS dpd1_30_count,
                                SUM(CASE WHEN COALESCE(p1.dpd, 0) BETWEEN 1 AND 30 AND COALESCE(p1.loan_status, '') <> 'N' THEN COALESCE(p1.exposure, l.loan_amount, 0) ELSE 0 END) AS dpd1_30_val,
                                COUNT(CASE WHEN COALESCE(p1.dpd, 0) BETWEEN 31 AND 60 AND COALESCE(p1.loan_status, '') <> 'N' THEN 1 END) AS dpd31_60_count,
                                SUM(CASE WHEN COALESCE(p1.dpd, 0) BETWEEN 31 AND 60 AND COALESCE(p1.loan_status, '') <> 'N' THEN COALESCE(p1.exposure, l.loan_amount, 0) ELSE 0 END) AS dpd31_60_val,
                                COUNT(CASE WHEN COALESCE(p1.dpd, 0) BETWEEN 61 AND 90 AND COALESCE(p1.loan_status, '') <> 'N' THEN 1 END) AS dpd61_90_count,
                                SUM(CASE WHEN COALESCE(p1.dpd, 0) BETWEEN 61 AND 90 AND COALESCE(p1.loan_status, '') <> 'N' THEN COALESCE(p1.exposure, l.loan_amount, 0) ELSE 0 END) AS dpd61_90_val,
